...
=======
#dataFrame <- read.csv()
dataFrame <- read.csv(pathFlavio)
>>>>>>> 562d324b46f7de7ddac0509badaf87309d7ed635
dataFrame
```
...
```{r}
#dataFrame$TotalNights <- paste(dataFrame$StaysInWeekNights, dataFrame$StaysInWeekendNights)
...
```{r collapse=TRUE}
#IsCanceled
freqTableIsCanceled <- cleanedList %>% group_by(IsCanceled = cleanedList$IsCanceled) %>% summarise(Count = n())
freqTableIsCanceled <- freqTableIsCanceled[order(freqTableIsCanceled$Count, decreasing = TRUE),]
totalSegments <- sum(freqTableIsCanceled$Count)
freqTableIsCanceled <- cbind(freqTableIsCanceled, Percentage = freqTableIsCanceled$Count/totalSegments * 100)
kable(freqTableIsCanceled, digits = 2, format = "pipe")
```
...
```
Erkenntniss:
Keine auserordentliche Erkenntniss
...
```
Erkenntniss:
Hauptsächlich kein Depot hinterlegt.
...
```
Erkenntniss:
Die Firma 223 ist ein grosser Kunde.
...
```
Erkenntniss:
Der Agent 240 betätigt über ein drittel der Reservationen.
...
```
Erkenntniss:
Die Daten enthalten mit grosser Wahrscheindlichkeit nur Teildaten der Jahre 2015 und 2017.
